...
// New 无向图 单源最短路径
func New() *dijkstra {
	return &dijkstra{nodes: make(map[string][]edge)}
}

func (g *dijkstra) AddEdge(origin, destiny string, weight int) {
...
func (g *dijkstra) GetEdges(node string) []edge {
	return g.nodes[node]
}

func (g *dijkstra) Dijkstra(origin, destiny string) (int, []string) {
	h := newHeap()
